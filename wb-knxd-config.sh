...
case ${layer2%%:*} in
    ft12|ft12cemi|ncn5120|tpuarts|tpuart)
        KNX_DEVICE=$(echo "$layer2" | cut -d':' -f2)
        ...
        ;;
    ip|ipt|iptn|tpuarttcp|ft12tcp|ft12cemitcp|ncn5120tcp|usb|dummy)
        # no validation
        LAYER2_ARGS="-b $layer2"
        ;;
    *)
        echo "Unknown layer-2 driver"
        ;;
esac
